...
if (!spot) return <></>;
  ...
  return (
    ...
    <h5><span><i className="fa-solid fa-star"></i></span>{spot.numReviews === 1 ? ` ${parseInt(spot.avgStarRating)}    ·   ${spot.numReviews} review`: spot.numReviews === 0 ? "New" :` ${parseInt(spot.avgStarRating)} · ${spot.numReviews} reviews` }</h5>
    </div>
    <button onClick={()=>alert("Feature Coming Soon")} className="reserve">Reserve</button>
    </div>
     </div>
     </div>
     <h3><span><i className="fa-solid fa-star"></i></span>{ spot.numReviews === 1 ? ` ${parseInt(spot.avgStarRating)}·${spot.numReviews} review`: spot.numReviews === 0 ? "New" :` ${parseInt(spot.avgStarRating)} · ${spot.numReviews} reviews` }</h3>
